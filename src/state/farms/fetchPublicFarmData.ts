...
type PublicFarmData = {
  ...
  multiplier: string
  depositfee: string
  StarDustPerBlock: SerializedBigNumber
  canharvest: boolean
}
...
const fetchFarm = async (farm: Farm): Promise<PublicFarmData> => {
  ...
  const [info, totalAllocPoint, StarDustPerBlock, startBlockHarvest] =
    pid || pid === 0
  ...
  const _StarDustPerBlock = StarDustPerBlock? new BigNumber(StarDustPerBlock).div(BIG_TEN.pow(9)) : BIG_ZERO
  ...
  return {
    ...
    StarDustPerBlock: _StarDustPerBlock.toJSON(),
    canharvest
  }
}
